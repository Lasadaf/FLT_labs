...
Случай 1:
```
[A] -> abc. //Вернуться по А?
[B] -> abc. //Вернуться по B?
```
...
Конфликт считается неразрешенным при lookahead = k, если не выполнено ни одно из соответствующих рассматриваемому случаю условий выше.
...
При этом объекты в кавычках явялются терминальными символами. Расстановка пробелов и табуляций может быть произвольной. Правила разделяются символом новой строки \n или набором символов \r\n. Должен присутствовать начальный нетерминал [S] (иначе в автомате будет всего лишь два состояния: `[♥] -> .[S]` и `[♥] -> [S].`). Ввод завершается при помощи пустой строки.
...
- Информация о конфликтах (если есть):
  + Участники конфликта;
  + Разрешается ли конфликт при k <= N; 
  + Если конфликт разрешается, при каком минимальном значении k это проиcходит.
- Дополнительное сообщение об отсутствии конфликтов, если их не было или они все были разрешены при k <= N.
